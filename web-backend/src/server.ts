...
const performConversionAsync = async (inputSource: string, isFile: boolean = false, conversionId: string) => {
  ...
  if (!isFile) {
    ...
    ffmpegCommand.inputOptions(["-protocol_whitelist", "file,http,https,tcp,tls,crypto"]);
  }
  ...
  ffmpegCommand
    ...
    .on("error", (err: any) => {
      ...
      if (err.message.includes("Invalid argument")) {
        userError =
          "Conversion failed: The M3U8 file format may be incompatible or the media segments are not accessible.";
...
const validateM3U8File = (
  filePath: string,
): { valid: boolean; error?: string; isMasterPlaylist?: boolean; bestStreamUrl?: string } => {
  try {
    ...
    if (isMasterPlaylist) {
      ...
      const bestStream = streams.reduce((best, current) => (current.bandwidth > best.bandwidth ? current : best));

      console.log(
        `Master playlist detected with ${streams.length} streams. Selected: ${
          bestStream.resolution || "unknown resolution"
        } (${bestStream.bandwidth} bps)`,
      );
